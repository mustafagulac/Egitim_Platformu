...
if ($_POST) {
    ...
    if ($_POST["sira"] != '' && $_FILES["dokumanIcon"]['name'] != '' && $_FILES["dokuman"]['name'] != '') {
        if ($_FILES['dokumanIcon']['error'] != 0) {
            ...
            $ekleSorgu = $baglanti->prepare(query: 'INSERT INTO python_dokuman SET dokumanIcon=:dokumanIcon, dokumanBaslik=:dokumanBaslik, dokumanKategori=:dokumanKategori, dokuman=:dokuman, aktif=:aktif, sira=:sira');
            ...
            if ($ekle) {
                ...
                if(value.isConfirmed){window.location.href='python_dokuman.php'}})</script>";
            }
        }
