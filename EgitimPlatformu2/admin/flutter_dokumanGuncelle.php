...
if ($_SESSION["yetki"] != "1" && $_SESSION["yetki"] != "2") {
    ...
    echo "<script> Swal.fire({title:'Hata!', text:'Yetkisiz kullanıcı', icon:'error', confirmButtonText:'Kapat'}).then((value)=>{
            if(value.isConfirmed){window.location.href='flutter_dokuman.php'}
        })
         </script>";
    exit;
}
                            ...
                            $id=$_GET['id'];
                            $sorgu = $baglanti->prepare(query: "select*from flutter_dokuman where id=:id");
                            $sorgu->execute(['id'=>$id]);
                            $sonuc = $sorgu->fetch();
...
if ($_POST) {
    ...
    if ($_POST["sira"] != '' && $_FILES["dokumanIcon"]['name'] != '' && $_FILES["dokuman"]['name'] != '') {
        if ($_FILES['dokumanIcon']['error'] != 0) {
            ...
            copy($_FILES['dokuman']['tmp_name'], to: '../document/flutter/' . strtolower($_FILES["dokuman"]['name']));
            unlink('../document/flutter/'.$sonuc['dokuman']);
    ...
    if($_POST["sira"] != '' && $hata=='')
    {
        $Sorgu = $baglanti->prepare(query: 'UPDATE flutter_dokuman SET dokumanIcon=:dokumanIcon,dokumanBaslik=:dokumanBaslik,dokumanKategori=:dokumanKategori,dokuman=:dokuman,sira=:sira,aktif=:aktif WHERE id=:id');
