...
if ($_SESSION["yetki"] != "1" && $_SESSION["yetki"] != "2") {
    ...
    exit;
                            ...
                            $id=$_GET['id'];
                            $sorgu = $baglanti->prepare(query: "select*from html_dokuman where id=:id");
                            $sorgu->execute(['id'=>$id]);
                            $sonuc = $sorgu->fetch();
...
if ($_POST) {
    ...
    if ($_POST["sira"] != '' && $_FILES["dokumanIcon"]['name'] != '' && $_FILES["dokuman"]['name'] != '') {
        if ($_FILES['dokumanIcon']['error'] != 0) {
            ...
            unlink('../document/html5/'.$sonuc['dokuman']);
    ...
    if($_POST["sira"] != '' && $hata=='')
    {
        $Sorgu = $baglanti->prepare(query: 'UPDATE html_dokuman SET dokumanIcon=:dokumanIcon,dokumanBaslik=:dokumanBaslik,dokumanKategori=:dokumanKategori,dokuman=:dokuman,sira=:sira,aktif=:aktif WHERE id=:id');
