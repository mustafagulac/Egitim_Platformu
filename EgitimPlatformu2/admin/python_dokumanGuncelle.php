...
if ($_SESSION["yetki"] != "1" && $_SESSION["yetki"] != "2") {
    ...
    echo "<script> Swal.fire({title:'Hata!', text:'Yetkisiz kullanıcı', icon:'error', confirmButtonText:'Kapat'}).then((value)=>{
            if(value.isConfirmed){window.location.href='python_dokuman.php'}
        })
         </script>";
    exit;
}
                            ...
                            $id=$_GET['id'];
                            $sorgu = $baglanti->prepare(query: "select*from python_dokuman where id=:id");
                            $sorgu->execute(['id'=>$id]);
                            $sonuc = $sorgu->fetch();
...
if ($_POST) {
    ...
    if ($_POST["sira"] != '' && $_FILES["dokumanIcon"]['name'] != '' && $_FILES["dokuman"]['name'] != '') {
        if ($_FILES['dokumanIcon']['error'] != 0) {
            ...
            copy($_FILES['dokuman']['tmp_name'], to: '../document/python/' . strtolower($_FILES["dokuman"]['name']));
            unlink('../document/python/'.$sonuc['dokuman']);
    ...
    if($_POST["sira"] != '' && $hata=='')
    {
        $Sorgu = $baglanti->prepare(query: 'UPDATE python_dokuman SET dokumanIcon=:dokumanIcon,dokumanBaslik=:dokumanBaslik,dokumanKategori=:dokumanKategori,dokuman=:dokuman,sira=:sira,aktif=:aktif WHERE id=:id');
