...
if ($_SESSION["yetki"] != "1" && $_SESSION["yetki"] != "2") {
    ...
    echo "<script> Swal.fire({title:'Hata!', text:'Yetkisiz kullanıcı', icon:'error', confirmButtonText:'Kapat'}).then((value)=>{
            if(value.isConfirmed){window.location.href='flutter_video.php'}
        })
         </script>";
    exit;
}
                            ...
                            $id=$_GET['id'];
                            $sorgu = $baglanti->prepare(query: "select*from flutter_video where id=:id");
                            $sorgu->execute(['id'=>$id]);
                            $sonuc = $sorgu->fetch();
...
if ($_POST) {
    ...
    if ($_POST["sira"] != '' && $_FILES["kucukResim"]['name'] != '' && $_FILES["video"]['name'] != '') {
        if ($_FILES['kucukResim']['error'] != 0) {
            ...
            copy($_FILES['kucukResim']['tmp_name'], to: '../images/thumbnail/flutter/' . strtolower($_FILES["kucukResim"]['name']));
            unlink('../images/thumbnail/flutter/'.$sonuc['kucukResim']);
            $foto=strtolower($_FILES["kucukResim"]['name']);
            copy($_FILES['video']['tmp_name'], to: '../videos/flutter/' . strtolower($_FILES["video"]['name']));
            unlink('../videos/flutter/'.$sonuc['video']);
    ...
    if($_POST["sira"] != '' && $hata=='')
    {
        $Sorgu = $baglanti->prepare(query: 'UPDATE flutter_video SET kucukResim=:kucukResim,kucukResimBaslik=:kucukResimBaslik,kucukResimKategori=:kucukResimKategori,video=:video,videoBaslik=:videoBaslik,videoAciklama=:videoAciklama,sira=:sira,aktif=:aktif WHERE id=:id');
            ...
            <div class="card-body">
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                        <img width="200" src="../images/thumbnail/flutter/<?= $sonuc['kucukResim']?>" alt=""> <br>
