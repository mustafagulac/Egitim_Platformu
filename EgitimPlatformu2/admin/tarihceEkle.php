...
if ($_SESSION["yetki"] != "1") {
    ...
    echo "<script> Swal.fire({title:'Hata!', text:'Yetkisiz kullanıcı', icon:'error', confirmButtonText:'Kapat'}).then((value)=>{
            if(value.isConfirmed){window.location.href='tarihce.php'}
        })
         </script>";
    exit;
}
...
if ($_POST) {
    ...
    if ($_POST["tarih"] != '' && $_POST["baslik"] != '' && $_POST["icerik"] != '' && $_FILES["foto"]['name'] != '') {
        if ($_FILES['foto']['error'] != 0) {
        ...
        } else if (file_exists(filename: '../assets/img/about/' . strtolower($_FILES["foto"]['name']))) {
            ...
            copy($_FILES['foto']['tmp_name'], to: '../assets/img/about/' . strtolower($_FILES["foto"]['name']));
            $ekleSorgu = $baglanti->prepare(query: 'INSERT INTO tarihce SET foto=:foto, tarih=:tarih, icerik=:icerik, baslik=:baslik');
