...
$id = $_GET['id'];
$sorgu = $baglanti->prepare(query: "select*from html_video where id=:id");
$sorgu->execute(['id' => $id]);
$sonuc = $sorgu->fetch();
...
if ($_POST) {
    ...
    if ($_POST["sira"] != '') {
        ...
        if ($_FILES["video"]['name'] != '') {
            if ($_FILES['video']['error'] != 0) {
                ...
                unlink('../videos/html5/' . $sonuc['video']);
                $video = strtolower($_FILES["video"]['name']);
            }
        }


        if ($_FILES["kucukResim"]['name'] != '') {
            if ($_FILES['kucukResim']['error'] != 0) {
                ...
                unlink('../images/thumbnail/html5/' . $sonuc['kucukResim']);
    ...
    if ($_POST["sira"] != '' && $hata == '') {
        $Sorgu = $baglanti->prepare(query: 'UPDATE html_video SET kucukResim=:kucukResim,kucukResimBaslik=:kucukResimBaslik,kucukResimKategori=:kucukResimKategori,video=:video,videoBaslik=:videoBaslik,videoAciklama=:videoAciklama,sira=:sira,aktif=:aktif WHERE id=:id');
            ...
            <div class="card-body">
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                        <img width="200" src="../images/thumbnail/html5/<?= $sonuc['kucukResim'] ?>" alt=""> <br>
