...
if ($_POST) {
    ...
    if ($_POST["sira"] != '' && $_FILES["kucukResim"]['name'] != '' && $_FILES["video"]['name'] != '') {
        if ($_FILES['kucukResim']['error'] != 0) {
            ...
            $ekleSorgu = $baglanti->prepare(query: 'INSERT INTO react_video SET kucukResim=:kucukResim, kucukResimBaslik=:kucukResimBaslik, kucukResimKategori=:kucukResimKategori, video=:video, videoBaslik=:videoBaslik, videoAciklama=:videoAciklama, aktif=:aktif, sira=:sira');
            ...
            if ($ekle) {
                ...
                if(value.isConfirmed){window.location.href='react_video.php'}})</script>";
            }
        }
